"! @testing BDEF:ZRAP100_R_TravelTP_THO
CLASS zrap100_tc_travel_eml_tho DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_tho IMPLEMENTATION.
  METHOD class_setup.

    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                              i_for_entities = VALUE #(
                                ( i_for_entity = 'ZRAP100_R_TravelTP_THO' ) ) ).
  ...
  METHOD create_with_action.
    ...
    SELECT * FROM zrap100_r_traveltp_tho INTO TABLE @DATA(lt_travel).
